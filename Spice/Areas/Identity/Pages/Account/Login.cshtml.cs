    ...
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var user = await _dp.Users.Where(m => m.Email == Input.Email).FirstOrDefaultAsync();
                    List<ShoppingCart> shoppingCarts = await _dp.ShoppingCarts.Where(m=>m.ApplicationUserId==user.Id).ToListAsync();
